(() => {
  ...
  function showDebugToast(msg, color) {
    ...
    toast.style.cssText = `position:fixed;top:10px;right:10px;z-index:2147483647;padding:8px 16px;background:${color || '#27ae60'};color:#fff;font-family:'Consolas',monospace;font-size:13px;border:2px solid #4a4a4a;box-shadow:2px 2px 0 #4a4a4a;`;
  ...
  function syncToWebApp(type, data) {
    showDebugToast('Sending ' + type + ': ' + JSON.stringify(data).substring(0, 60), '#2980b9');
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        showDebugToast('ERROR: ' + chrome.runtime.lastError.message, '#c0392b');
      } else {
        showDebugToast('Sent OK (no response expected)', '#27ae60');
      }
    });
  }
  ...
  function createPanel() {
    ...
    style.textContent = `
      ...
      .la-dict-save-btn {
        ...
        background: #d5f5e3; border: 2px solid #27ae60; color: #27ae60;
        font-family: 'Press Start 2P', monospace; font-size: 8px;
        cursor: pointer; text-align: center; box-shadow: 2px 2px 0 #27ae60; width: 100%;
      }
      .la-dict-save-btn:hover { background: #abebc6; }
      .la-dict-save-btn:active { box-shadow: none; transform: translate(2px, 2px); }
  ...
  function setupControls() {
    document.getElementById('la-minimize-btn').addEventListener('click', () => {
      ...
      const btn = document.getElementById('la-minimize-btn');
      btn.textContent = panel.classList.contains('minimized') ? '□' : '─';
      btn.title = panel.classList.contains('minimized') ? '展开' : '最小化';
    });
  ...
  async function lookupFreeDictionary(word) {
    try {
      ...
      html += '</div><button class="la-dict-save-btn" data-word="' + entry.word + '" data-phonetic="' + phonetic.replace(/"/g, '&quot;') + '">⭐ 收藏单词</button><button class="la-dict-ai-btn" data-word="' + entry.word + '">🤖 Ask AI</button>';
      html += '<div class="la-dict-source">Free Dictionary API</div>';
      dictCard.innerHTML = html;
      const dictSaveBtn = dictCard.querySelector('.la-dict-save-btn');
      if (dictSaveBtn) {
        dictSaveBtn.addEventListener('click', () => {
          syncToWebApp('word', { word: dictSaveBtn.dataset.word, lang: currentLang, phonetic: dictSaveBtn.dataset.phonetic || '' });
          dictSaveBtn.textContent = '✓ 已收藏';
        });
      }
      ...
      html += `<button class="la-dict-save-btn" data-word="${word}">⭐ 收藏单词</button><button class="la-dict-ai-btn" data-word="${word}">🤖 Ask AI</button>`;
      dictCard.innerHTML = html;
      dictCard.querySelector('.la-dict-save-btn').addEventListener('click', function() {
  ...
  async function lookupWiktionary(word, lang) {
    try {
      ...
      html += '</div><button class="la-dict-save-btn" data-word="' + word + '">⭐ 收藏单词</button><button class="la-dict-ai-btn" data-word="' + word + '">🤖 Ask AI</button>';
  ...
  function stripSaveMarker(text) {
    return text.replace(/\[SAVE:[^\]]*\]/g, '');
  }

  function renderMd(text) {
    let html = stripSaveMarker(text)
  ...
  function createSubtitleItem(text, lang, startTime, endTime) {
    ...
    saveBtn.className = 'la-replay-btn';
    saveBtn.innerHTML = '⭐';
    saveBtn.title = '收藏句子';
    ...
    metaDiv.appendChild(langLabel); metaDiv.appendChild(replayBtn); metaDiv.appendChild(saveBtn); metaDiv.appendChild(timeLabel);
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'CHAT_DONE') {
      ...
      if (msgs.length > 0) {
        ...
        const saveMatch = finalText.match(/\[SAVE:([^\]]+)\]/);
        if (saveMatch) {
          const saveItems = saveMatch[1].split(';').map(item => {
            const parts = item.split('|');
            return { word: (parts[0] || '').trim(), notes: (parts[1] || '').trim() };
          }).filter(item => item.word);
          saveItems.forEach(item => {
            syncToWebApp('word', { word: item.word, lang: currentLang, notes: item.notes });
          });
          if (saveItems.length > 0) {
            addChatMsg('system', '✓ 已收藏 ' + saveItems.length + ' 个单词: ' + saveItems.map(d => d.word).join(', '));
          }
        }
        msgs[0].innerHTML = renderMd(finalText);
        msgs[0].classList.remove('streaming');
        chatHistory.push({ role: 'assistant', content: stripSaveMarker(finalText) });
        const sendBtn = document.getElementById('la-chat-send');
  ...
  document.getElementById('la-minimize-btn').textContent = '□';
  document.getElementById('la-minimize-btn').title = '展开';
})();
